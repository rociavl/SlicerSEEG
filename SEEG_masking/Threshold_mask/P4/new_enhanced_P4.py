...
def process_original_ctp(enhanced_volumes, volume_array, threshold_tracker):
    ...
    threshold =2506
    enhanced_volumes['DESCARGAR_OG_gaussian_volume_og_2016'] = np.uint8(enhanced_volumes['OG_gaussian_volume_og'] > threshold)
...
def process_roi_plus_gamma_after(enhanced_volumes, final_roi, threshold_tracker):
    ...
    threshold = 189
    enhanced_volumes['DESCARGAR_2_LOG_189'] = np.uint8(enhanced_volumes['2_LOG'] > threshold)
    ...
    # Threshold sharpened volume
    threshold = 0.840
    enhanced_volumes['DESCARGAR_2_sharpening_2_trial_0.660'] = np.uint8(enhanced_volumes['2_sharpening_2_trial'] > threshold)
...
def process_original_idea(enhanced_volumes, threshold_tracker):
    ...
    # Threshold gamma corrected volume
    threshold = 89
    enhanced_volumes['DESCARGAR_ORGINAL_IDEA_gamma_correction_38'] = np.uint8(enhanced_volumes['ORGINAL_IDEA_gamma_correction'] > threshold)
    ...
    # Threshold gamma corrected volume
    threshold = 53
    enhanced_volumes['DESCARGAR_ORIGINAL_IDEA_GAMMA_2_53'] = np.uint8(enhanced_volumes['ORIGINAL_IDEA_GAMMA_2'] > threshold)
...
def process_first_try(enhanced_volumes, roi_volume, threshold_tracker):
    ...
    threshold = 1705
    enhanced_volumes['DESCARGAR_FT_gaussian_1705'] = np.uint8(enhanced_volumes['FT_gaussian'] > threshold)
    ...
    threshold = 900
    enhanced_volumes['DESCARGAR_FT_RESTA_TOPHAT_GAUSSIAN_900'] = np.uint(enhanced_volumes['FT_RESTA_TOPHAT_GAUSSIAN'] > threshold)
    ...
    threshold= 19
    enhanced_volumes['DESCARGAR_FT_sharpened_19'] = np.uint8(enhanced_volumes['FT_sharpened'] > threshold)
...
# Main execution
inputVolume = slicer.util.getNode('4_CTp.3D')  
inputROI = slicer.util.getNode('patient4_mask_5')  # Brain Mask 
# Output directory
outputDir = r"C:\Users\rocia\Downloads\TFG\Cohort\Enhance_ctp_tests\P4\TH45_histograms_FILTERED"
...
#exec(open(r'C:\Users\rocia\AppData\Local\slicer.org\Slicer 5.6.2\SEEG_module\SEEG_masking\Threshold_mask\P4\new_enhanced_P4.py').read())