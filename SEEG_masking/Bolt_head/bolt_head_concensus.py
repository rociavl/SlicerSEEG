...
CONFIG = {
    ...
    'output_dir': r"C:\Users\rocia\Downloads\TFG\Cohort\Bolt_heads\P2_entry_concensus_pdf_features_lines"
}
...
class BoltHeadDetector:
    ...
    def analyze_threshold_effects_3d(self, volume_array, brain_mask_array, volume_helper):
        """Optimized threshold effects analysis using matrix operations with weighted thresholds."""
        threshold_values = [2400]
        results = []
        ...
        print("Performing optimized threshold analysis with weighted thresholds...")
        
        # Initialize voting mask and region accumulator
        voting_mask = np.zeros_like(volume_array, dtype=np.uint8)
        consensus_regions = []
        
        # Process each threshold with weights
        for threshold in threshold_values:
            start_time = time.time()
            
            # Apply threshold and processing pipeline (matrix operations only)
            ...
            # Apply weighting - give more votes to 2350 and 2920
            weight = 2 if threshold in [2400] else 1
            voting_mask += filtered_mask * weight
            
            # Collect statistics (without full regionprops)
            stats = {
                'threshold': threshold,
            ...
            print(f"Processed threshold {threshold} (weight={weight}) in {time.time()-start_time:.2f}s")
        
        # Calculate total weight for majority calculation
        total_weight = sum(r['weight'] for r in results)
        
        # Now create consensus regions from the voting mask
        consensus_mask = voting_mask >= (total_weight*0.35)  # Majority vote considering weights
        consensus_labeled = label(consensus_mask)
        consensus_regions = regionprops(consensus_labeled)
        
        # Add consensus metrics to results
        for r in results:
            r['consensus_voxels'] = np.sum(consensus_mask)
            r['consensus_regions'] = len(consensus_regions)
        
        # Visualization of just the consensus results (much faster)
...
# exec(open(r'C:\Users\rocia\AppData\Local\slicer.org\Slicer 5.6.2\SEEG_module\SEEG_masking\Bolt_head\bolt_head_concensus.py').read())